...
class Model(object):
    ...
    def init_graph(self):
        ...
        x_shape = [config[mode].batch_size, config.num_agents, config.num_items]
        adv_shape = [config.num_agents, config[mode].num_misreports, config[mode].batch_size, config.num_agents, config.num_items]
        adv_var_shape = [ config[model].num_misreports, config[mode].batch_size, config.num_agents, config.num_items]
        u_shape = [config.num_agents, config[mode].num_misreports, config[mode].batch_size, config.num_agents]
        ...
        self.adv_mask[np.arange(config.num_agents), :, :, np.arange(config.num_agents), :] = 1.0
        
        self.u_mask = np.zeros(u_shape)
        self.u_mask[np.arange(config.num_agents), :, :, np.arange(config.num_agents)] = 1.0
        ...
        # Misreports
        misreports = self.get_misreports(self.x, self.adv_var, adv_shape)
        
        # Get mechanism for true valuation: Allocation and Payment
        alloc, pay = self.net.forward(x)
        ...
        # Regret Computation
        u_mis = tf.reshape(utility_mis, u_shape) * u_mask
        utility_true = tf.tile(utility, [config.num_agents * config.train.num_misreports, 1])
        excess_from_utility = tf.nn.relu(tf.reshape(utility_mis - utility_true, u_shape) * u_mask)
        ...
        if mode is "train":
            ...
            with tf.variable_scope('lag_var'):
                self.w_rgt = tf.Variable(np.ones(config.num_agents).astype(np.float32) * w_rgt_init_val, 'w_rgt')
            ...
            # Val ops
            val_opt = tf.train.AdamOptimizer(config.val.gd_lr)
            ...
            # Metrics
            self.metrics = [revenue, rgt_mean, rgt_penalty, lag_loss, loss_1, tf.reduce_mean(w_rgt), update_rate]
            ...
            tf.summary.scalar('w_rgt_mean', tf.reduce_mean(w_rgt))
            ...
            opt = tf.train.AdamOptimizer(config.test.gd_lr)
            test_gd_step = opt.minimize(loss, var_list = [adv_var])
            test_reset_opt = tf.variables_initializer(opt.variables())
        ...
        self.assign_op = tf.assign(adv_var, adv_init)
    ...
    def train(iter):
        ...
        while iter < (self.config.train.max_iter):
            ...
            if iter % self.config.train.update_frequency == 0:
                self.sess.run(self.lag_update, feed_dict = {self.x:X})
                

            if iter % self.config.train.up_op_frequency == 0:
                self.sess.run(self.up_op)
            ...
            if (iter % self.config.train.print_iter) == 0:
                # Train Set Stats
                summary = self.sess.run(self.merged, feed_dict = {self.x: X})
    ...
    def test(self, X_tst = None, ADV_tst = None):
        ...
        for i in range(self.config.test.num_batches):
            ...
            sess.run(assign_op, feed_dict = {self.adv_init: ADV})
            ...
            metric_vals = sess.run(self.metrics, feed_dict = {x: X})
            
            if X_tst is not None:
                A, P = sess.run([self.alloc, self.pay], feed_dict = {x:X})
                alloc_tst[perm, :, :] = A
                pay_tst[perm, :] = P
        ...
        if X_tst is not None:
            np.save(os.path.join(config.dir_name, 'alloc_tst_' + str(iter)), alloc_tst)
            np.save(os.path.join(config.dir_name, 'pay_tst_' + str(iter)), pay_tst)
